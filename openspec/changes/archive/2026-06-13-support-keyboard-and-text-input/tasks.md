...
- [x] 5.1 启动服务端（hdc install + hdc fport），启动客户端连接设备，确认连接后键盘焦点自动获取（无需手动点击镜像区域）
- [x] 5.2 测试字母键（a-z）输入，确认设备焦点文本框正确显示对应字符
- [x] 5.3 测试数字键（0-9）输入，确认设备焦点文本框正确显示对应数字
- [x] 5.4 测试方向键（↑↓←→），确认设备端焦点导航行为正确
- [x] 5.5 测试 Enter/Tab/Space/Backspace/Delete/Escape 键，确认功能正确
- [x] 5.6 测试数字小键盘键（Numpad0-9、Numpad+、NumpadEnter 等），确认功能正确
- [x] 5.7 测试侧边栏文本输入面板：输入文本（含中文）→ 点击发送，确认设备焦点文本框显示对应文本
- [x] 5.8 测试剪贴板恢复：发送文本前复制一段内容到设备剪贴板，发送后确认剪贴板恢复为原内容
- [x] 5.9 测试断连后键盘事件不再转发（`connState != connected` 时按键无响应）
...
- [x] 8.1 WiFi 连接默认参数调整：最短边 2160、码率 6 Mbps、帧率 15 fps
- [x] 8.2 码率-分辨率联动更新：2160p → 6 Mbps（原 12 Mbps），1080p 保持 4 Mbps
- [x] 8.3 添加 `shared_preferences` 依赖，实现用户手动选择的分辨率/帧率持久化
- [x] 8.4 连接时优先使用保存的参数，无保存值则按连接类型（WiFi/USB）使用默认值
- [x] 8.5 `flutter analyze` 确认无编译错误
