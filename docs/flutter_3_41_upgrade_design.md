...
2026-07-30 经用户另行授权修改和部署服务端后，进一步定位到未回显并非控制协议丢包：

- 设备日志显示服务端能收到 15/20 fps 请求，并成功重建采集与编码器。
- `ScreenCaptureEncoder.cpp` 在 H.264 启动成功后固定把 `cfg_.frameRate` 覆盖为
  20，导致后续视频配置包始终向客户端报告 20 fps。
- 修复为保留 ArkTS 调用方传入、且已经用于初始化编码器的帧率，不再做固定覆盖。
- 服务端版本号仍保持 1.0.2，修复版 HAP 已在 OpenHarmony API 23 设备覆盖部署。
- 新增真机集成回归用例，验证同一连接内 15→20→15 fps 双向切换时，每次均收到
  目标视频配置和后续视频帧。
- 修复后录制与截图关联回归通过：5.385 秒 H.264 MP4 可由 `ffmpeg` 完整解码并由
  `ffplay` 播放；3840×2160 PNG 格式及画面检查正常。
